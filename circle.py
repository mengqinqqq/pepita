...
def quantify(filenames):
	file_results = do_quantify5(filenames)
...
def do_quantify4(filenames):
	return {filename: _quantify_4_plus(filename) for filename in filenames}

def do_quantify5(filenames):
	pool = Pool(8)
	return {filename: result for filename, result in zip(filenames, pool.map(_quantify_4_plus, filenames))}
...
def _quantify_4_plus(filename):
	img_array = imageio.imread(filename)[:,:,1]
	mask = imageops.get_fish_mask(filename.replace('CH1', 'CH4'), True) # to produce stepwise pictures of the process
	# mask = imageops.get_fish_mask(filename.replace('CH1', 'CH4'))

	img_array = np.minimum(img_array, np.where(mask == 255, 2**16-1, 0).astype(np.uint16))
	return img_array.sum(dtype=np.uint64, where=(img_array>5_000))
